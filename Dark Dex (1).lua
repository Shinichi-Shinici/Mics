...
local function main()
		...
		for i = 1,maxNodes do
			...
			if node then
                ...
                if (isa(obj,"LocalScript") or isa(obj,"Script")) and obj.Disabled then
                    ...
                    entry.Indent.Icon.Image = "rbxassetid://16183417031"
                    entry.Indent.Icon.ImageRectSize = Vector2.new(6.7, 16)
                    entry.Indent.Icon.ImageRectOffset = Vector2.new((rmdEntry and rmdEntry.ExplorerImageIndex or 0) * 7, 0)
				end
	...
	Explorer.ShowRightClick = function()
		...
		if presentClasses["LuaSourceContainer"] then context:AddRegistered("READ_MODULE") end
		if presentClasses["LuaSourceContainer"] then context:AddRegistered("READ_TBL") end
	...
	Explorer.InitRightClick = function()
		...
		local VIEW_OBJECT_DB = true
		context:Register("VIEW_OBJECT",{Name = "View Object", IconMap = Explorer.ClassIcons, Icon = 5, OnClick = function()
			VIEW_OBJECT_DB = not VIEW_OBJECT_DB
			if VIEW_OBJECT_DB then
				local sList = selection.List
				local isa = game.IsA
		...
		context:Register("READ_MODULE",{Name = "Read Module", IconMap = Explorer.MiscIcons, Icon = "ViewScript", OnClick = function()
			local scr = selection.List[1] and selection.List[1].Obj
			if scr then ScriptViewer.ReadModule(scr) end
		end})

		context:Register("READ_TBL",{Name = "Read Table", IconMap = Explorer.MiscIcons, Icon = "ViewScript", OnClick = function()
			local scr = selection.List[1] and selection.List[1].Obj
			if scr then ScriptViewer.ReadTable(require(scr)) end
		end})
...
local function main()
    ...
    ScriptViewer.ReadModule = function(scr)
		local success, source = pcall(getgenv().readmodule or function() end, scr)
		if not success or not source then source, PreviousScr = "-- DEX - Source failed to decompile", nil else PreviousScr = scr end
		codeFrame:SetText(source)
		window:Show()
	end

	ScriptViewer.ReadTable = function(scr)
		local success, source = pcall(getgenv().tabledDump or function() end, scr)
		if not success or not source then source, PreviousScr = "-- DEX - Source failed to decompile", nil else PreviousScr = scr end
		codeFrame:SetText(source)
		window:Show()
	end
...
Main = (function()
	...
	Main.CreateIntro = function(initStatus) -- TODO: Must theme and show errors
		local gui = create({
			...
			{11,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Creator",Parent={2},Position=UDim2.new(1,-110,1,-20),Size=UDim2.new(0,105,0,20),Text="Developed by Moon\n Edited by acsu123",TextColor3=Color3.new(1,1,1),TextSize=14,TextXAlignment=1,}},
